...
- Added a `/yearly-high-low` route that computes yearly average `High` and `Low` prices for each of the three time-split dataframes and returns a combined line chart (inline PNG) comparing the periods.